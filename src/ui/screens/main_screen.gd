...
func _on_auth_completed(_success: bool, _user_id: String) -> void:
	CloudSave.start_after_auth()
	CloudSave.load_finished.connect(_on_data_loaded)


func _on_data_loaded(_success: bool) -> void:
	# Veri yuklendi, UI guncelle
	_switch_tab("home")


func _on_auth_failed(error: String) -> void:
	push_error("Auth failed: %s" % error)
	# Offline modda devam — yerel veri ile
	if GameData.player_id.is_empty():
		GameData.initialize_new_player("local_player", "Player")
...
func _switch_tab(tab_name: String) -> void:
	...
	for name in tab_buttons:
		var btn: Button = tab_buttons[name]
		if name == tab_name:
...
func _on_mission_completed(_mission_id: String, success: bool, _rewards: Dictionary) -> void:
	if success:
		_show_floating_text("BASARILI!", ThemeConstants.SUCCESS_COLOR)
	else:
		_show_floating_text("BASARISIZ!", ThemeConstants.DANGER_COLOR)
...
func _show_floating_text(text: String, color: Color) -> void:
	...
	label.position = Vector2(0, feedback_container.size.y / 2)
